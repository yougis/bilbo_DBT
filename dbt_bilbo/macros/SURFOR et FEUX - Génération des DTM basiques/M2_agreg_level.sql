...
-- Récupération des listes par dimension depuis les variables définies dans "dbt_project"
    ...
    {% for column in columns %}
        {%- if column.name.lower() not in var('list_spatiale') and column.name.lower() not in var('list_mesure') and column.name.lower() not in var('list_technique') -%}
            {%- do dim_theme.append(column.name) -%}
        {%- endif -%}
    {% endfor %}

    {%- set list_select = dim_spatiale + dim_theme -%}
    {%- set list_ordered =  dim_spatiale + dim_theme + dim_mesure -%}
    {%- set list_ordered_avec_technique = dim_spatiale + dim_theme + dim_mesure + dim_technique -%}
