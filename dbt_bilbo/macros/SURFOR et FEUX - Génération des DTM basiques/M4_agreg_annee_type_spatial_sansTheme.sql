{% macro agreg_annee_type_spatial_sanstheme(schema, table_name, granu_tempo) %}
...
-- Récupération des listes par dimension depuis les variables définies dans "dbt_project"
    ...
    {% for column in columns %}
        {% if column.name.lower() in var('list_tech') %}
            {%- do dim_technique.append(column.name) -%}
        {% endif %}
    {% endfor %}

    {% for column in columns %}
        {%- if column.name.lower() not in var('list_temporelle') and column.name.lower() not in var('list_spatiale') and column.name.lower() not in var('list_mesure') and column.name.lower() not in var('list_tech') -%}
    ...
    all_combinations AS (
        ...
        JOIN 
            distinct_levels l ON s.level = l.type_spatial
    ),
