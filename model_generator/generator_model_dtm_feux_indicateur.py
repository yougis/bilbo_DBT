    ...
    for scrow in fdf.schema.unique():
        ...
        for obrow in (fdf[fdf['schema'] ==scrow].object_name).unique():
            ...
            for obtrow in (fdf[(fdf['schema'] ==scrow) & (fdf['object_name'] ==obrow)].object_type).unique():
                ...
                filename = obrow+'.sql'
                with open(Path(npath/filename), 'w') as fp:
                    if 'nb_theme_indic' in cols:
                        fp.write("{% set theme = ' ' %}\n")
                        fp.write("{% set champ_theme = ' ' %}\n")
                        fp.write("\n")
                        fp.write("{{ rename_dtm_indicateur(theme, champ_theme) }}\n")
                        fp.write("\n")
                        fp.write("{{ indicateur(theme, champ_theme) }}\n")
                    else:
                        fp.write("{% set theme = ' ' %}\n")
                        fp.write("\n")
                        fp.write("{{ rename_dtm_indicateur_sanstheme(theme) }}\n")
                        fp.write("\n")
                        fp.write("{{ indicateur_sanstheme(theme) }}\n")
